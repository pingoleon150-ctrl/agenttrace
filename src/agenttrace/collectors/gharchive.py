...
class GHArchiveHourCollector(Collector):
    ...
    async def collect(self) -> AsyncIterator[Observation]:
        ...
        async with httpx.AsyncClient(
            headers=headers, timeout=max(60.0, self.settings.timeout_seconds)
        ) as client:
            ...
            with gzip.GzipFile(fileobj=io.BytesIO(response.content)) as gz:
                for index, raw_line in enumerate(gz):
                    ...
                    observation = github_event_to_observation(
                        event, retrieval_method="gharchive-hourly"
                    )
                    if observation:
    ...
    def _normalize_hour(value: str) -> str:
        ...
        datetime.strptime(value, "%Y-%m-%d-%H").replace(tzinfo=UTC)
        return value

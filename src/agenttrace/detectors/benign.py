...
AUTOMATION_RE = re.compile(
    r"(?:release|ci|build|deploy|mirror|sync|dependency|version bump)", re.IGNORECASE
)
...
def detect_benign_automation(observations: list[Observation]) -> list[Signal]:
    ...
    known = sum(
        ...
        if o.actor.lower() in KNOWN_BOTS or o.actor.lower().endswith("[bot]")
    )
    ...
    return [
        Signal(
            ...
            evidence=[
                f"known_bot_fraction={known_fraction:.2f}",
                f"automation_text_fraction={text_fraction:.2f}",
            ],
        )
    ]
